...
public abstract class BaseScenario {
    ...
    protected void postJson(String path, String json, Consumer<Response> consumer) {
        ...
        Invocation invocation = getTarget(path)
    ...
    protected String waitForAppToDeploy(UUID appInstanceId) {
        ...
        while (attempt < maxRetries) {
            Map stateMap = doGet(
                    "hub-web-api/app-instance/" + appInstanceId + "/state", Map.class,
                    (response, value) ->
                    Assert.assertEquals(
    ...
    protected void populateServiceConfigurationParams(
        ...
        final Map siteSpaces = doGet(
                "hub-web-api/site-topology/" + siteId.toString(),
                Map.class
        );
        ...
        final Map supportedConfigurationsOnSite = doGet(
                "hub-web-api/test-dev/site/" + siteId.toString() +
                        "/app-template-configuration/" + appTemplateId.toString(),
                Map.class
        );
        ...
        parseDataService(
                tokenValues,
                supportedConfigurationsOnSite,
                singeDataServiceName,
                "dsb");
    }
    ...
    protected void doGetAndValidateJson(String url, String jsonResourcePath, final Map<String, String> tokens) {
        doGet(
                url,
                String.class,
                (r, value) -> assertJsonEquals(readResourceAsString(jsonResourcePath, tokens), value));
    }
